...
class RelResultadoExamesWORD
{
    ...
    public static function gerar(Request $request, Response $response)
    {
        ...
        $section->addText('Resultado de Exames', ['size' => 16, 'bold' => true], ['alignment' => \PhpOffice\PhpWord\SimpleType\Jc::CENTER]);

        // Adiciona a tabela
        $table = $section->addTable();
        $table->addRow(null, ['spaceAfter' => 0]);
        $table->addCell(6000)->addText('Nome do Animal:', ['bold' => true, 'size' => 10]);
        ...
        $table->addRow();
        $table->addCell(2000)->addText('Clínica Veterinária:', ['bold' => true, 'size' => 10]);
        $table->addCell(8500)->addText('Hospital Universitário UNESC');
        ...
        $table->addCell(2000)->addText('CRMV:', ['bold' => true, 'size' => 10]);
        $table->addCell(4000)->addText($DadosFicha->getVeterinarioRemetente()->getNrCRMV());

        $table->addRow();
        $table->addCell(2000)->addText('Data:', ['bold' => true, 'size' => 10]);
        $table->addCell(8500)->addText(date('d/m/Y', strtotime($DadosFicha->getData())));

        $section->addTextBreak(1);
        $section->addText('RESULTADO DE EXAME NECROSCÓPICO', ['size' => 18, 'bold' => false], ['alignment' => \PhpOffice\PhpWord\SimpleType\Jc::CENTER]);
        $section->addTextBreak(1);
